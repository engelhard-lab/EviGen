...
extract the token-level softmax probability P(Yes) as the predicted risk score.

The prompt uses assistant prefill: the assistant turn starts with "Answer: "
...
def parse_report_from_completion(raw_text: str) -> str:
    ...
    # Fallback: strip the first line (Yes/No) and return the rest as-is
...
def build_system_prompt(pos_str: str, neg_str: str) -> str:
    ...
    return (
        ...
        f"order), you (1) answer a binary ({pos}/{neg}) question about whether the "
        "patient will die within 1 year of their last hospital discharge, and (2) "
        "produce a conservative, evidence-grounded clinical report identifying the "
        "top 5 predictive factors drawn ONLY from the provided history.\n\n"
        "Hard rules:\n"
...
def main():
    ...
    parser.add_argument("--verbalizer-style", choices=["yesno", "plusminus", "reserved"],
                        default="yesno",
    ...
    parser.add_argument("--truncate-direction", choices=["recent", "early"],
                        default="recent",
                        help="When patient_text exceeds --max-input-tokens, which "
                             "end of the chronological note list to keep. 'recent' "
                             "(default) keeps the most recent notes; 'early' keeps "
                             "the earliest. Must match the value used at training.")
    ...
    parser.add_argument("--enable-thinking", choices=["auto", "true", "false"],
                        ...
                        help="Pass enable_thinking=<bool> to apply_chat_template; "
                             ...
                             "verbalizer runs use 'false' so position-0 is Yes/No "
                             "rather than <think>.")
    ...
    for p in patients:
        prompt, ntok, truncated, kept, total_notes = fit_prompt_to_budget(
            ...
            direction=args.truncate_direction,
    ...
    if args.rope_scaling_json:
        ...
        # Also override max_position_embeddings so the rotary embedding
        # table is sized for the extended context (vLLM 0.19 NaN at
        # positions > native max otherwise).
