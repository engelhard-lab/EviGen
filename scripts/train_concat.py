"""
Training script for EviGenConcat (concatenation fusion variant).

Usage:
    python train_concat.py --config configs/config_concat.yaml --run-name concat_v1

This is a thin wrapper that reuses the training infrastructure from
scripts/train.py but swaps in EviGenConcat from model_evigen_concat.py.
"""
import argparse
import copy
import json
...
from evigen_dynamicquery.model_evigen_concat import EviGenConcat, TemperatureScheduler
...
def train_model(
    ...
    for epoch in range(cfg.num_epochs):
        ...
        val_metrics = evaluate_model(model, val_loader, cfg, use_sampling=False)
...
def evaluate_model(
    ...
    with torch.no_grad():
        for note_chunks, note_mask, code_chunks, code_mask, labels in data_loader:
            ...
            mask_q = gating_info["mask"]
            dyn_active = mask_q[:, 1:].sum(dim=1).float()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train EviGen-Concat (concatenation fusion)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/config_concat.yaml",
...
def make_run_stem(run_name: str) -> str:
    ...
    if not run_name:
        run_name = datetime.now().strftime('concat_iris_%Y%m%d_%H%M%S')
...
if __name__ == "__main__":
    ...
    print(f"Model: EviGenConcat (concatenation fusion)")
    if cfg.num_code_queries > 0:
        print(f"Classifier input dim: {2 * cfg.emb_dim} (note [{cfg.emb_dim}] || code [{cfg.emb_dim}])")
    else:
        print(f"Classifier input dim: {cfg.emb_dim} (note-only, no concatenation)")
